...
new _vue2.default({
  ...
  methods: {
    ...
    //画像をランダムに取得する
    getReelImg: function getReelImg() {
      var randomIndex = Math.floor(Math.random() * this.result.length);
      this.image = this.result[randomIndex].slot;
      this.getResult = this.result[randomIndex];
    },

    //スロットスタート
    slotStart: function slotStart() {
      var _this = this;
    ...
    //初期化
    init: function init() {
      this.maxbetFlg = false, this.stopbtnFlg = false, this.leverFlg = false;
    },
...
//MEMO
// refsの部分（各リールの画像だとかボタンだとか。）がうまく取れてないからリールとボタンが独立してない
